...
const ManagementDashboard: React.FC<ManagementDashboardProps> = ({ 
  ...
  const openBankAccountModal = (bank: IspProfile['bankAccounts'][0] | null, index: number | null) => {
      if (bank && index !== null) {
          setEditingBankAccount({ bank, index });
          setBankAccountFormData(bank);
      } else {
          setEditingBankAccount(null);
          setBankAccountFormData({ bankName: '', accountNumber: '', accountName: '' });
      }
  ...
  const openWaTemplateModal = (template: WhatsAppTemplate | null) => {
      if (template) {
          setEditingWaTemplate(template);
          setWaTemplateFormData(template);
      } else {
          setEditingWaTemplate(null);
          setWaTemplateFormData({ name: '', template: '' });
      }
            ...
            <div className="space-y-8">
                ...
                <div className="bg-slate-800 rounded-lg p-6">
                    ...
                    <div className="space-y-4">
                        ...
                        {waTemplates.length === 0 && <p className="text-slate-400 text-center py-4">Belum ada templat yang ditambahkan.</p>}
                    </div>
                </div>
            </div>
         );
